...
export default class LinkUI extends Plugin {
  ...
  _createFormView() {
    ...
    formView.uploadStorageButtonView
      .bind('isEnabled')
      .to(linkCommand, 'isEnabled', value => !value);
    formView.saveButtonView.bind('isEnabled').to(linkCommand);

    // Perform handler and execute link command after clicking the "Upload" button.
    formView.uploadStorageButtonView.on('done', async (_, files) => {
      const { uploadHandler } = editor.config.get('link');

      if (typeof uploadHandler !== 'function') {
        throw Error('CKeditor5: link.uploadHandler in configuration is not a function');
      }
      ...
      if (url == null) {
        ...
        throw Error(
          'CKeditor5: link.uploadHandler in configuration should return "string" or "null"'
        );
      }
